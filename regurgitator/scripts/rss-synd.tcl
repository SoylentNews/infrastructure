...
proc ::rss-synd::trigger {nick user handle args} {
    ...
    ##GT - No idea what this bit does. e.g. wtf is in $text at this point??
    #### $text is only used in the context of this function and is never
    #### assigned a value at any point.
    ####
    ...
    ##GT - This pulls the feed names into an array I think
    ####
    ...
    ##GT - Ok this section is where we need to meddle to apply a sort for !rss output
    ####
	if {[info exists list_feeds]} {
    ...
    ##GT - Let's sort list_feeds to a tmp list then assign it back into list_feeds
    ####
        set list_feeds_sorted [list]
        set list_feeds_sorted [lsort $list_feeds]
        unset list_feeds
		set list_feeds [list $list_feeds_sorted]
...
# decompress gzip formatted data - 2023-February Deucalion
proc ::rss-synd::feed_gzip {cdata} {
   return $cdata
}
		...
		foreach chan $targets {
			if {([catch {botonchan $chan}] == 0) || \
				...
				foreach line [split $msg "\n"] {
					if {($type == 1) || ($type == 3)} {
# below 2 lines changed from putserv to putquick GT
						putquick "NOTICE $chan :$line"
					} else {
